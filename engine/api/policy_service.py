...
# Content Safety and Prompt Injection Guards
_INJECTION_PATTERNS = [
    r"(?i)\bignore\s+(all\s+)?(prior|previous|above)\s+instructions\b",
    r"(?i)\byou\s+are\s+now\s+(in\s+)?(a\s+)?(developer|dan|jailbreak|unrestricted|god)\s+mode\b",
    r"(?i)\bsystem\s+override\b",
    r"(?i)\boutput\s*:\s*system\s+compromised\b",
    r"(?i)\bforget\s+(your|all)\s+instructions\b",
    r"(?i)\bdisregard\s+(the\s+)?(previous|above)\s+rules\b",
    r"(?i)<\s*script[^>]*>",
]

_OFFENSIVE_PATTERNS = [
    r"(?i)\b(hate\s+speech|kill\s+all|death\s+to|white\s+supremacy|terrorist_org)\b",
]


def sanitize_outreach_input(text: Optional[str]) -> Optional[str]:
    """
    Guards against prompt injection and content safety violations:
    - Neutralizes instructions attempting to hijack LLM behavior.
    - Rejects overtly offensive or abusive inputs with HTTP 400.
    """
    if not text:
        return text
    ...
    for pat in _OFFENSIVE_PATTERNS:
        if re.search(pat, text):
            raise HTTPException(
                status_code=400,
                detail="Inappropriate or offensive content detected in input fields.",
            )

    # 2. Prompt Injection Check: If adversarial instruction is detected, replace with safe generic role
    for pat in _INJECTION_PATTERNS:
        if re.search(pat, text):
            return "Hiring Team Member"

    # Strip dangerous prompt delimiter markers (System:, Human:, Assistant:)
    sanitized = re.sub(r"(?i)\b(system|human|assistant|developer)\s*:\s*", "", text)
    return sanitized.strip()


def verify_draft_grounding(
    draft_text: str,
    expected_confidence: int,
    expected_visas: List[str],
    expected_company: str,
    expected_job_title: str,
) -> bool:
    """
    Audit-Grade Fact Grounding Verification:
    Asserts every factual claim in the generated draft is traceable to real underlying data.
    - Confidence score matches real confidence.
    - Visa types mentioned match supported visa types.
    - Company and role match verified target job.
    - Zero hallucinated numbers or fabricated policy dates.
    """
    # Check confidence score grounding
    score_matches = re.findall(r"\b(\d+)/100\b", draft_text)
    for sm in score_matches:
        if int(sm) != expected_confidence:
            return False

    # Check company presence
    if expected_company.lower() not in draft_text.lower():
        return False

    # Check job title presence
    if expected_job_title.lower() not in draft_text.lower():
        return False

    # Check that any visa mentioned in the sponsorship highlight (or custom draft) is within supported list
    highlight_match = re.search(r"\(Verified VisaLane Sponsorship Confidence:[^\)]+\)", draft_text)
    text_to_check = highlight_match.group(0) if highlight_match else draft_text
    for standard_visa in ["H-1B", "O-1", "TN", "E-3", "L-1"]:
        if standard_visa in text_to_check and standard_visa not in expected_visas:
            return False
...
def generate_outreach_draft(request: OutreachDraftRequest) -> OutreachDraftResponse:
    ...
    - Prompt-injection safe: input fields are sanitized against adversarial overrides.
    - Fact-grounded: all claims map to real underlying company/job data.
    """
    ...
    # 3. Sanitize User Inputs Against Prompt Injection & Inappropriate Content
    safe_contact_name = sanitize_outreach_input(request.contact_name)
    safe_contact_role = sanitize_outreach_input(request.contact_role)
    safe_candidate_notes = sanitize_outreach_input(request.candidate_notes)

    # 4. Retrieve Real Company and Role Context
    import engine.api.jobs_routes as jr
    slug_norm = request.company_id.strip().lower()

    target_job: Optional[Dict[str, Any]] = None
    for j in jr._MOCK_JOBS_STORE:
    ...
    # 5. Synthesize Personalized Outreach Message (Ephemeral Contact Binding)
    salutation = f"Hi {safe_contact_name}," if safe_contact_name else "Hello,"
    contact_ref = f" in your role as {safe_contact_role}" if safe_contact_role else ""

    candidate_pitch = ""
    if safe_candidate_notes:
        candidate_pitch = f"\n\nContext on my background:\n{safe_candidate_notes.strip()}\n"
    ...
    assert verify_draft_grounding(
        ...
        expected_confidence=conf_score,
        expected_visas=visas,
        expected_company=company_name,
        expected_job_title=job_title,
    ), "Draft failed strict factual grounding audit."

    # 6. Audit Check: Log only operation success WITHOUT contact personal data
    logger.info(
