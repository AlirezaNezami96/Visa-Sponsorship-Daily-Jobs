"""Workable ATS source adapter."""
from __future__ import annotations

import logging
from typing import List

from job_radar.fetchers.ats import fetch_workable
...
class WorkableAdapter(SourceAdapter):
    """Adapter for Workable ATS job boards."""
    ...
    async def fetch(self, config: JobSearchConfig) -> List[Job]:
        ...
        if config.company_urls:
            for url in config.company_urls:
                slug = extract_slug_from_url(url, "workable")
                if slug:
                    slugs.add(slug)
        ...
        if not slugs and not config.company_urls and not config.company_names:
            curated = get_curated_companies_for_ats("workable")
            for c in curated:
        ...
        for slug in slugs:
            try:
                jobs = fetch_workable(slug=slug, days_back=days_back)
                all_jobs.extend(jobs)
                if len(all_jobs) >= config.max_per_source:
                    all_jobs = all_jobs[:config.max_per_source]
                    break
            except Exception as e:
                logger.debug("Error fetching workable for %s: %s", slug, e)

        return all_jobs

    async def fetch_by_company(self, company_url: str, config: JobSearchConfig) -> List[Job]:
        slug = extract_slug_from_url(company_url, "workable")
        if not slug:
            return []
        days_back = config.posted_within_days or 30
        return fetch_workable(slug=slug, days_back=days_back)
