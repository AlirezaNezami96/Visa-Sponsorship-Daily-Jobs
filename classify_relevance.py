...
Scores candidate jobs that passed keyword pre-filtering to eliminate boilerplate false positives
(e.g., general SWE roles at AI companies or non-remote disguised listings).
...
from config_loader import RadarConfig, get_config
...
"""
    ...
    if not parsed:
        from filter import match_track
...
def classify_and_filter_jobs(
    ...
    Returns:
        (qualified_jobs, classification_stats)
    """
    cfg = config or get_config()
    ...
    for job in jobs:
        ...
        # If job has explicit remote flag or remote in location, allow region_restricted/worldwide
        ...
        # Attach classification results to job
